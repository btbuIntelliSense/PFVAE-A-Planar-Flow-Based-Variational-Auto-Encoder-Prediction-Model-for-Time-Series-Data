...
def multinomial_loss_function(x_logit, x, z_mu, z_var, z_0, z_k, ldj, args, beta=1.):
    ...
    # make integer class labels 创建整型类标签
    target = (x * (num_classes-1)).long()

    # - N E_q0 [ ln p(x|z_k) ]
    # sums over batch dimension (and feature dimension) 批处理维度(和特性维度)上的总和
...
def binary_loss_array(recon_x, x, z_mu, z_var, z_0, z_k, ldj, beta):
    """
...
def calculate_loss(x_mean, x, z_mu, z_var, z_0, z_k, ldj, args, beta):
    """
    Picks the correct loss depending on the input type.
    """

    if args.input_type == 'binary':
        loss, rec, kl = binary_loss_function(x_mean, x, z_mu, z_var, z_0, z_k, ldj, beta)
        bpd = 0.
