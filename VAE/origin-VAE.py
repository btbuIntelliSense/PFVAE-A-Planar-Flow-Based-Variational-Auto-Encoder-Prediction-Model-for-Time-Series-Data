...
from optimization.training import train, evaluate,evaluate2
...
parser.add_argument('-li', '--log_interval', type=int, default=1, metavar='LOG_INTERVAL',
...
parser.add_argument('-nf', '--num_flows', type=int, default=1,
...
np.random.seed(args.manual_seed) # 42.796616864409536
...
def run(args, kwargs):
    ...
    def figure_error(y_test, test_x_mean):
        ...
        data4 = np.array(test_x_mean).reshape(-1, 1)
        data5 = np.array(y_test).reshape(-1, 1)
        RMSE = math.sqrt(mean_squared_error(test_x_mean.reshape(-1, 1), y_test.reshape(-1, 1)))  # 均方误差
        MSE = mean_squared_error(test_x_mean.reshape(-1, 1), y_test.reshape(-1, 1))
        MAE = mean_absolute_error(test_x_mean.reshape(-1, 1), y_test.reshape(-1, 1))
        SMAPE = 2.0 * np.mean(np.abs(y_test.reshape(-1, 1) - test_x_mean.reshape(-1, 1)) / (
                    np.abs(y_test.reshape(-1, 1)) + np.abs(test_x_mean.reshape(-1, 1)))) * 100
    ...
    def save_result(y_test, x_mean):
        ...
        x_mean.to_csv("result/new_preds_test.csv", index=False, header=['pre_feature'])
