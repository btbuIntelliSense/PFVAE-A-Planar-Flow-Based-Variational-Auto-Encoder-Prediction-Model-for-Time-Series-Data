...
class NN(nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Conv1d(in_channels=3,out_channels=24,kernel_size=(3,3))
...
plt.legend(['CNN-LSTM_predict', 'CNN-LSTM_real'], loc='best')
plt.show()


def save_result(y_test, x_mean):
    y_test = pd.DataFrame(data=y_test.reshape(-1, 1))
    y_test.to_csv(r"..\code\VAE-LSTM-Planar\result\CNN-LSTM_y_test.csv", index=False, header=['y_test'])
    x_mean = pd.DataFrame(data=x_mean.reshape(-1, 1))
    x_mean.to_csv(r"..\code\VAE-LSTM-Planar\result\CNN-LSTM_test_pred.csv", index=False, header=['pre_feature'])
