...
class NN(nn.Module):
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(input_size=feature_num, hidden_size=24, batch_first=False, bidirectional=False)
        # self.lstm2 = nn.LSTM(input_size=24, hidden_size=24, batch_first=False, bidirectional=False)
...
plt.legend(['LSTM_predict', 'LSTM_real'], loc='best')
plt.show()


def save_result(y_test, x_mean):
    y_test = pd.DataFrame(data=y_test.reshape(-1, 1))
    y_test.to_csv(r"..\code\VAE-LSTM-Planar\result\LSTM_y_test.csv", index=False, header=['y_test'])
    x_mean = pd.DataFrame(data=x_mean.reshape(-1, 1))
    x_mean.to_csv(r"..\code\VAE-LSTM-Planar\result\LSTM_test_pred.csv", index=False, header=['pre_feature'])
